/*
 * PATIENT APPOINTMENT.JS — Shows patient's appointment history
 */
import { API_ENDPOINTS } from "./config/config.js";
import { makeRequest, getToken } from "./util.js";
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const appointments = await makeRequest(API_ENDPOINTS.APPOINTMENTS);
        if (!appointments || appointments.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="noPatientRecord">No appointments found.</td></tr>';
            return;
        }

        appointments.forEach((apt) => {
            const row = document.createElement("tr");
            const date = apt.appointmentTime ? new Date(apt.appointmentTime).toISOString().split("T")[0] : "N/A";
            row.innerHTML = `
                <td>${date}</td>
                <td>${apt.id}</td>
                <td>${apt.patient?.id || "N/A"}</td>
                <td><button class="prescription-btn" onclick="viewPrescription(${apt.id})">📋</button></td>
            `;
            tbody.appendChild(row);
        });
    } catch (error) {
        tbody.innerHTML = '<tr><td colspan="4" class="noPatientRecord">Failed to load appointments.</td></tr>';
    }
});
